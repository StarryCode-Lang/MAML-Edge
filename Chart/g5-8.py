...
OUTPUT_STEM = "fig5_8_accuracy_vs_model_size"
...
def load_ablation_rows():
    ...
    for row in rows:
        parsed.append({
            ...
            "model_size_mb": to_float(row["model_size_mb"]),
        })
...
def plot_profile(ax, profile, rows):
    ...
    x_values = [row["model_size_mb"] for row in rows]
    ...
    for index, row in enumerate(rows):
        ...
        ax.annotate(
            ...
            xy=(row["model_size_mb"], row["accuracy_percent"]),
            xytext=offset,
...
def main():
    ...
    ax.set_title("Accuracy-Model Size Trade-off Across Compression Stages", fontsize=12)
